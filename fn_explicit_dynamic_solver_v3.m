function [history_output, field_output, force_output] = fn_explicit_dynamic_solver_v3(...
    K, M, time, ...
    forcing_indices, forcing_functions, ...
    disp_indices, disp_functions, ...
    history_indices, field_output_every_n_frames, use_diagonal_lumped_mass_matrix, varargin)
%SUMMARY
...
%   forcing_indices - p-element vector of global matrix indices at which forcing functions
%   will be applied
%   forcing_functions - p x n matrix of forces to apply
%   disp_indices - q-element vector of global matrix indices at which
%   displacements will be imposed
%   disp_functions - q x n matrix of displacements to apply
%   output every field_output_every_n_frames (set to inf to get no field
%   output)
%   history_nodes - q-element vector of global matrix indices for complete 
...
if use_diagonal_lumped_mass_matrix
    tmp = sum(M);
    M = spdiags(sum(M).', 0, size(K,1), size(K,2)); 
    inv_M = 1 ./ M;
end
...
accn(:, 2:end - 1) = (disp_functions(:, 3:end) - 2 * disp_functions(:, 2:end-1) + disp_functions(:, 1:end-2)) / time_step ^ 2;
...
for ti = 1:length(time)
    ...
    u_previous(disp_indices) = disp_functions(:, ti);
    ...
    %At nodes where displacement enforced, acceleration is calculated
    %directly
    u_dot_dot_previous(disp_indices) = accn(:, ti);
    %work out velocity
